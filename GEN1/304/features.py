...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^304 bcstarw (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^304 stopbcstarw$'))
