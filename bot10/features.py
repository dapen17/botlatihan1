...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^dsex hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex stop$'))
    ...
    # Tes koneksi bot
    @client.on(events.NewMessage(pattern=r'^dsex ping$'))
    async def ping_handler(event):
        await event.reply("🏓 Pong! Bot aktif.")
        message_count[get_today_date()] += 1

    # Broadcast pesan ke semua chat kecuali blacklist
    @client.on(events.NewMessage(pattern=r'^dsex bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex bcstarw (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex stopbcstarw$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex help$'))
    async def help_handler(event):
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. dsex hastle [pesan] [waktu][s/m/h/d]\n"
    ...
    # Atur auto-reply
    @client.on(events.NewMessage(pattern=r'^dsex setreply (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^dsex stopall$'))
