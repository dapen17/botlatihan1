...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^206hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206stop$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206ping$'))
    async def ping_handler(event):
        """Tes koneksi bot."""
        await event.reply("🏓 Pong! Bot aktif.")
        message_count[get_today_date()] += 1

    @client.on(events.NewMessage(pattern=r'^206bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206bcstarw (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206stopbcstarw$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^206help$'))
    async def help_handler(event):
        """Tampilkan daftar perintah."""
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. 206hastle [pesan] [waktu][s/m/h/d]\n"
