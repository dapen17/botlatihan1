...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^febri ping$'))
    async def ping_handler(event):
        """Tes koneksi bot."""
        await event.reply("\U0001F3D3 Pong! Bot aktif.")
        message_count[get_today_date()] += 1

    @client.on(events.NewMessage(pattern=r'^febri bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^febri bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^febri stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^febri setreply (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^febri stopall$'))
    ...
    @client.on(events.NewMessage(pattern=r'^febri help$'))
    async def help_handler(event):
        """Tampilkan daftar perintah."""
        help_text = (
            "\U0001F4AC Daftar Perintah:\n"
            "febri ping - Cek status bot\n"
            "febri bcstar <pesan> - Kirim broadcast ke semua chat\n"
            "febri bcstargr<nomor> <interval> <pesan> - Kirim broadcast ke grup tertentu\n"
            "febri stopbcstargr<nomor> - Hentikan broadcast ke grup tertentu\n"
            "febri setreply <pesan> - Set auto-reply untuk chat masuk\n"
            "febri stopall - Reset semua pengaturan\n"
            "febri help - Menampilkan daftar perintah"
        )
